...
test('Draft.eta awaits promised values before yielding', async t => {
	const draft = Draft.eta(Promise.resolve('awaited value'));
	const result1 = await draft.next();
	t.false(result1.done);
	t.is(result1.value, 'awaited value');
});
...
test('Controlflow.first awaits promised source values', async t => {
	const cf = Controlflow.from(Promise.resolve('test'));
	t.is(await cf.first(), 'test');
});
...
test('controlflow.transform propagates synchronous errors from the callback', async t => {
	const cf = Controlflow.from('test');
	const transformed = cf.transform(() => {
		throw new Error('sync failure');
	});
	await t.throwsAsync(transformed.first(), { message: 'sync failure' });
});
...
test('controlflow.through runs a side effect and preserves the value', async t => {
	let observed = '';
	const cf = Controlflow.from('test');
	const passed = cf.through(x => {
		observed = x;
	});
	t.is(await passed.first(), 'test');
	t.is(observed, 'test');
});

test('controlflow.through propagates synchronous errors from the callback', async t => {
	const cf = Controlflow.from('test');
	const passed = cf.through(() => {
		throw new Error('side effect failure');
	});
	await t.throwsAsync(passed.first(), { message: 'side effect failure' });
});
